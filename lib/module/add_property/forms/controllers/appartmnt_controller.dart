...
import '../../../../services/get_properties_services.dart';


class AppartmentController extends GetxController {
  AddPropertyController addPropertyController = Get.find();
  GlobalKey<FormState> appartmentFormKey = GlobalKey<FormState>();
  ...
  List<String> refundableItem = ['Yes', 'No'];

  String _refundable = '';
  String get refundable => _refundable;

  set refundable(String type) {
    _refundable = type;
    update();
  }
  ...
  void postAppartment() {
    if (appartmentFormKey.currentState!.validate()) {
      if (city == '' &&
          state == '' &&
